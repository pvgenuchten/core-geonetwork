(function () {
  ...
  var module = angular.module('search_filter_tags_directive', ['gn_thesaurus_service', 'pascalprecht.translate']);
  module.filter('decodeURIComponent', function () {
    return window.decodeURIComponent;
  });

  module.directive('searchFilterTags', ['$location', 'gnThesaurusService', '$q', '$cacheFactory', '$browser',
    function ($location, gnThesaurusService, $q, $cacheFactory, $browser) {
      ...
      var generateCurrentFilters = function (filters, currentFilters, controller) {
        ...
        newLocationsPromise.then(function (locations) {
          ...
          angular.forEach(currentFilters, function (oldF, index) {
            if (newFacets.length > 0) {
              if (oldF.type == 'facet' &&
                ...
                var label = getLabelForFacet(oldF.key, oldF.value, controller.getSearchResults());
              ...
              } else if (oldF.type === 'geometry' && searchInGeometries(oldF, locations) > 0) {
                newFilters = newFilters.concat({
                  type: 'geometry',
                  key: oldF.key,
                  value: oldF.value
                });
              } else if (oldF.type === 'boolean' && oldF.value === true && filters[oldF.key] !== undefined) {
                newFilters = newFilters.concat({
                  type: 'boolean',
                  key: oldF.key,
                  value: true,
          ...
          for (var filterKey in filters) {
            if (filterKeyIsTaggable(filterKey) && !(searchInFilters(newFilters, filterKey, getTypeForKey(filterKey)))) {
              var newFilter = createNewFilter(filterKey, filters[filterKey]);
            ...
            if (filterKey === 'facet.q') {
              angular.forEach(newFacets, function (nf, index) {
                if (!searchInFilters(newFilters, nf.dimension, 'facet')) {
                  newFilters = newFilters.concat(createNewFilterFromFacets(nf, controller));
                }
              });
            }
            if (filterKey === 'geometry') {
              angular.forEach(locations, function (newLocation) {
                if (!searchInFilters(newFilters, newLocation.key, 'geometry')) {
      ...
      var getLabelForFacet = function (key, value, searchResults) {
        ...
        var currentDimension = $.grep( searchResults.dimension, function (elem, index) {
      ...
      var createNewFilterFromFacets = function (facet, controller) {
        ...
        var newFilter = {};
        var result = [];
        var label = getLabelForFacet(facet.dimension, facet.value, controller.getSearchResults());
      ...
      var filterKeyIsTaggable = function (filterKey) {
        var filtersNotTaggables = ['bboxes', 'fast', 'facet.q', 'from', 'geometry',
          'resultType', 'sortBy', 'sortOrder', '_owner', '_isTemplate',
          'to', '_content_type', 'ownRecords'];
      ...
      var getLocationsPromise = function (geometryParam) {
        var prefix = 'region:';
        var locations = [];
        if (!geometryParam || geometryParam.indexOf(prefix) !== 0 || geometryParam === prefix) {
          return $q.all();
        }
        var locationList = geometryParam.substring(prefix.length).split(/\s*,\s*/);
        var promises = [];
        $.each(locationList, function (idx) {
          promises.push(getKeywordFromUri(locationList[idx]).then(function (kw) {
            locations.push(
              {
                value: kw.label,
                key: kw.props.uri,
      ...
      var getKeywordFromUri = function (uri) {
        var defer = $q.defer();
        if (!cache.get(uri)) {
          gnThesaurusService.lookupURI('external.place.administrativeAreas', uri).then(function (keyword) {
            if (keyword) {
              var kw = {};
      ...
      var getSearchParameters = function (useLocation, locationProvider, controller) {
        if (useLocation) {
      ...
      var setSearchParameter = function (useLocation, locationProvider, controller, paramKey, paramValue) {
        if (useLocation) {
      ...
      return {
        ...
        templateUrl: '../../catalog/views/ngr/templates/' +
        ...
        link: function (scope, element, attrs, ngSearchFormCtrl) {
          ...
          scope.$watch(function () {
            return getSearchParameters(useLocationParameters, $location, ngSearchFormCtrl);
          }, function (newFilters, oldVal, currentScope) {
            generateCurrentFilters(newFilters,
              currentScope.currentFilters, ngSearchFormCtrl).then(function (calculatedFilters) {
            ...
            if (filter.type === 'facet') {
              var newFacets = $.grep(scope.currentFilters,
                function (element, index) {
                  return element.type === 'facet' && element.key !== filter.key;
                });
              var newFacetQParam = buildQParam(newFacets);
              if (newFacetQParam == '') {
                newFacetQParam = null;
              }
              setSearchParameter(useLocationParameters, $location, ngSearchFormCtrl, 'facet.q', newFacetQParam);
              // $location.search('facet.q', newFacetQParam);

            } else if (filter.type === 'geometry') {
              var newLocations = $.grep(scope.currentFilters,
                function (element, index) {
                  return element.type === 'geometry' && element.key !== filter.key;
                }
              );
              var newGeometryParam = buildGeometryParam(newLocations);
              setSearchParameter(useLocationParameters, $location, ngSearchFormCtrl, 'geometry', newGeometryParam);
              // $location.search('geometry', newGeometryParam);
            } else if (filter.type === 'boolean') {
              setSearchParameter(useLocationParameters, $location, ngSearchFormCtrl, filter.key, null);
